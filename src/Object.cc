...
Value Object::message(const Message &m) {
    if (m.message == "_init") {
        ...
        case OTHER: KillOther(dynamic_cast<Other *>(this)); break;
...
void Object::setAttrChecked(const std::string &key, const Value &val) {
    ...
    if (key == "name") {
        ...
        if (result == VALID_OK) {
            ...
            std::string reason;
            if (result == VALID_UNKNOWN_KEY)
                reason = "attribute name unknown";
            else if (result == VALID_ACCESS_DENIED)
            ...
            for (ObjectList::iterator oit = ol.begin(); oit != ol.end(); ++oit) {
                if (*oit != nullptr) {
                    ...
                    if (obj_action != "nop" && (*oit)->validateMessage(obj_action, messageValue)) {
                        if (secure)
                            PerformSecureAction(this->getId(), false, (*oit)->getId(), obj_action,
                                                messageValue);
                        else
...
double Object::squareDistance(const Object *) const {
    return 1e15;  // infinity in enigma world
}

bool Object::isSouthOrEastOf(const Object *) const {
    return true;
}
