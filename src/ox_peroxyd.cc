...
Stone* PerOxydLoader::make_stone(int type, int x, int y) {
    ...
    switch (type) {
        ...
        case 0x3b:
            ...
            break;
        default: st = OxydLoader::make_stone(type, x, y); break;
    }
    return st;
}
...
LP_PerOxyd::LP_PerOxyd(OxydLib::DatFile* dat, bool twoplayers)
    : LevelPack_Oxyd(OxydLib::OxydVersion_PerOxyd, dat, twoplayers ? 100 : 0, twoplayers ? 199 : 99,
              twoplayers) {
}

void LP_PerOxyd::load(const OxydLib::Level& level) {
    LoaderConfig c(needs_twoplayers(), get_gamemode(), peroxyd_floor_map, peroxyd_item_map,
            peroxyd_stone_map);
    c.id_timer = 0x2e;
    c.id_laser1 = 0x3e;
...
bool LP_PerOxyd::hasEasymode(size_t index) {
    static const bool easymode_map[200] = {
            // 1-100 single, 101 - 200 netplay
...
const char* oxyd::peroxyd_stone_map[256] = {
        ...
        0,
        0,
        0, // PerOxyd stone 0x01 - 0x08   oxyd stones
        0,
        0,
        0,
        0,
        ...
        0,
        0,
        0,                   // PerOxyd stone 0x08 - 0x10   oxyd stones
