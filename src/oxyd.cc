...
namespace {
    Direction direction_oxyd2enigma(OxydLib::Direction odir) {
        switch (odir) {
            ...
            default: fprintf(stderr, "Unknown OxydLib-direction %i!\n", int(odir)); return NODIR;
        }
    }

    GameType to_gametype(OxydVersion ver) {
    ...
    GridLoc to_gridloc(const SignalLocation& a) {
        assert(a.getGridType() >= GridType_First && a.getGridType() <= GridType_Last);
        static GridLayer tab[3] = {GRID_FLOOR, GRID_STONES, GRID_ITEMS};
        return GridLoc(tab[a.getGridType()], GridPos(a.getX(), a.getY()));
    }

    std::string patchfile_name(GameType t, int index, bool twoplayers) {
        std::string patchfile = "levels/patches/";
        switch (t) {
...
CommandString::CommandString(const std::string& cmd) : m_cmd(cmd), m_iter(m_cmd.begin()) {
}
...
OxydLoader::OxydLoader(const Level& level_, const LoaderConfig& config_)
    : level(level_), config(config_) {
    harmless_medi = false;
}
...
Stone* OxydLoader::make_laser(int type) {
    ecl::Assert<XLevelLoading>(
            type >= 0 && type <= 2, "Oxyd supports only three different lasers per level");

    const Laser& laser = level.getLaser(type);

    Direction dir = direction_oxyd2enigma(laser.getDir());
    bool on = laser.getOn();

    Stone* st = nullptr;
    if (dir != NODIR) {
        std::string lasername("st_laser");
...
Item* OxydLoader::make_item(int type, int x, int y) {
    ...
    Language lang = Language_English;
...
void OxydLoader::load_stones() {
    ...
    // Only shuffle if no scramble is defined in level data (e.g., oxyd1->lvl11, oxyd1->lvl69).
...
void OxydLoader::scramble_puzzles() {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        AddScramble(GridPos(si.getX(), si.getY()), direction_oxyd2enigma(si.getDir()));
    }
}
        ...
        switch (marble.getMarbleType()) {
            ...
            case MarbleType_Jack:
                ...
                if (!minfo.is_default(MI_FORCE)) {
                    ...
                    Log << "Set jack strength to " << force << std::endl;
                }
                if (!minfo.is_default(MI_RANGE)) {
                    double range = minfo.get_value(MI_RANGE)
                            / 32.0; // value seems to contain distance in pixels
                    ac->setAttr("range", range);
                    Log << "Set jack range to " << range << std::endl;
                }
                break;
            ...
            case MarbleType_Rotor: {
                ...
                Log << "rotor strength " << force << std::endl;
                Log << "rotor range " << range << std::endl;
            ...
            case MarbleType_Bug: ac = MakeActor("ac_bug"); break;
            default:
                Log << "Unhandled actor type " << int(marble.getMarbleType()) << std::endl;
                break;
...
void OxydLoader::parse_specials() {
    ...
    for (size_t i = 0; i < special.size(); ++i) {
        ...
        std::string args = special[i].substr(1);
        switch (cmd.get_char()) {
            case 'y':; // use black and white stones with hole
                break;
            ...
            case 'T': server::HoleForce = cmd.get_int(0, 4000, 40) / 40.0; break;

            default: Log << "special " << i << ": '" << special[i] << "'\n"; break;
        }
    }
}
...
LevelPack_Oxyd::LevelPack_Oxyd(
        OxydVersion ver, DatFile* dat, int idx_start, int idx_end, bool twoplayers)
    ...
    for (int i = idx_start; i <= idx_end; i++) {
        if (!m_datfile->getLevel(i).empty()) {
            ...
            lev::Proxy* aProxy = lev::Proxy::registerLevel("#" + get_name() + "#" + txt,
                    "#" + get_name(), ecl::strf("Import %s %d", get_name().c_str(), nlevels),
                    ecl::strf("%s #%d", get_name().c_str(), nlevels + 1), "Dongleware", 1, 1,
                    has_easymode(i), get_gametype());
...
GameMode LevelPack_Oxyd::get_gamemode() const {
    return (server::GetDifficulty() == DIFFICULTY_EASY ? GameMode_Easy : GameMode_Hard);
}

std::string LevelPack_Oxyd::get_name() const {
    static const char* names1p[] = {
            "Oxyd 1", "Oxyd magnum", "Magnum Gold", "Per.Oxyd", "Oxyd extra"};
    static const char* names2p[] = {"Oxyd 1 (2p)", "", "", "Per.Oxyd (2p)", "Oxyd extra (2p)"};
    OxydVersion v = get_version();
    return level_index[0] > 99 ? names2p[v] : names1p[v];
}

int LevelPack_Oxyd::get_default_location() const {
    static const int location1p[] = {90100, 90700, 90800, 90300, 90500};
    static const int location2p[] = {90200, 0, 0, 90400, 90600};
    OxydVersion v = get_version();
...
void LP_OxydExtra::load(const Level& level) {
    LoaderConfig c(needs_twoplayers(), get_gamemode(), oxydextra_floor_map, oxydextra_item_map,
            oxydextra_stone_map);
    //     c.id_timer  = 0x2e;
    c.id_laser1 = 0x3e;
...
LP_OxydMagnum::LP_OxydMagnum(OxydVersion version, DatFile* dat)
    : LevelPack_Oxyd(version, dat, 0, (version == OxydVersion_OxydMagnumGold) ? 120 : 182, false) {
}

void LP_OxydMagnum::load(const Level& level) {
    LoaderConfig c(needs_twoplayers(), get_gamemode(), oxydmag_floor_map, oxydmag_item_map,
            oxydmag_stone_map);
    c.id_timer = 0x33;
    c.id_laser1 = 0x44;
    PerOxydLoader(level, c).load();
...
MarbleInfo::MarbleInfo(const Marble& marble) {
    const std::string& data = marble.getData(
            server::GetDifficulty() == DIFFICULTY_EASY ? GameMode_Easy : GameMode_Hard);
    size_t from = 0;
    int idx = 0;
        ...
        if (par_open != std::string::npos) {
            ...
            if (par_close != std::string::npos) {
                ...
                if (par_close == par_open + 1) {
                    ...
                    value[idx++] = atoi(
                            data.substr(par_open + 1, par_close - par_open - 1).c_str());
                }
            } else {
...
MarbleInfo::~MarbleInfo() {
    for (int idx = 0; idx < MAX_MARBLE_INFO_FIELDS; ++idx) {
        if (!interpreted[idx] && !is_default(idx)) {
            Log << "MarbleInfo[" << idx << "]=" << get_value(idx) << " is not used yet."
                << std::endl;
        }
    }
}
...
GameInfo::GameInfo() : ver(OxydVersion_Invalid), datFile(nullptr), m_present(false) {
}

GameInfo::~GameInfo() {
}

GameInfo::GameInfo(OxydVersion ver_, const std::string& game_, const std::string& datfile_name_,
        const bool searchDAT)
    : ver(ver_), game(game_), m_present(false) {
    std::string alt_datfile_name = "levels/legacy_dat/" + datfile_name_;
    std::string fname;
    if (searchDAT
            && (app.resourceFS->findFile(datfile_name_, datFilePath)
                    || app.resourceFS->findFile(alt_datfile_name, datFilePath))) {
        Log << "Found " << game << " data file\n";
        m_present = true;
        openDatFile();
...
void GameInfo::openDatFile() {
    ...
    readFile(datFilePath, &data);

    datFile = std::make_unique<DatFile>();

    std::string errmsg;
    if (!parseDatFile(data, ver, datFile.get(), &errmsg)) {
        Log << "Error loading " << datFilePath << ": " << errmsg << std::endl;
        datFile.reset();
        m_present = false;
    } else {
        Log << "Loaded " << datFilePath << std::endl;
    }
}
...
lev::Index* GameInfo::makeLevelIndex(bool twoPlayers) {
    ...
    if (twoPlayers
            && (ver == OxydVersion_OxydExtra || ver == OxydVersion_OxydMagnum
                    || ver == OxydVersion_OxydMagnumGold)) {
        return nullptr; // no twoplayer levels available
    }

    switch (ver) {
        case OxydVersion_Oxyd1: return new LP_Oxyd1(datFile.get(), twoPlayers);
        case OxydVersion_OxydExtra: return new LP_OxydExtra(datFile.get());
        case OxydVersion_PerOxyd: return new LP_PerOxyd(datFile.get(), twoPlayers);
        case OxydVersion_OxydMagnum:
        case OxydVersion_OxydMagnumGold: return new LP_OxydMagnum(ver, datFile.get());
...
namespace {
    std::vector<std::unique_ptr<GameInfo>> games;
}
...
void Init(bool searchDAT) {
    ...
    games[OxydVersion_Oxyd1] = std::make_unique<GameInfo>(
            OxydVersion_Oxyd1, "Oxyd 1", "oxyd1ibm.dat", searchDAT);
    games[OxydVersion_OxydMagnum] = std::make_unique<GameInfo>(
            OxydVersion_OxydMagnum, "Oxyd magnum", "oxydmibm.dat", searchDAT);
    games[OxydVersion_OxydMagnumGold] = std::make_unique<GameInfo>(
            OxydVersion_OxydMagnumGold, "Oxyd magnum gold", "oxydmgg.dat", searchDAT);
    games[OxydVersion_OxydExtra] = std::make_unique<GameInfo>(
            OxydVersion_OxydExtra, "Oxyd extra", "oxydex.dat", searchDAT);
    games[OxydVersion_PerOxyd] = std::make_unique<GameInfo>(
            OxydVersion_PerOxyd, "Per.Oxyd", "peroxyd.dat", searchDAT);
}
...
bool FoundOxyd(OxydVersion ver) {
    return games[ver]->is_present();
}

bool InitOxydSoundSet(OxydVersion ver) {
    GameInfo& gi = *(games[ver]);

    if (!gi.is_present())
        return false; // not installed -> use enigma soundset

    static const char* oxydsounds[] = {"OXBLOOP.SDD", "OXBOING.SDD", "OXBOLD.SDD", "OXCRACK.SDD",
            "OXCRASH1.SDD", "OXCRASH2.SDD", "OXCUT.SDD", "OXDROP.SDD", "OXEXIT.SDD", "OXFINITO.SDD",
            "OXINTRO.SDD", "OXINVENT.SDD", "OXINVROT.SDD", "OXJUMP.SDD", "OXKLICK1.SDD",
            "OXKLICK2.SDD", "OXKLICK3.SDD", "OXKLICK4.SDD", "OXKLICK5.SDD", "OXKLICK6.SDD",
            "OXKLIRR.SDD", "OXLASER.SDD", "OXMAGIC.SDD", "OXMAGIC2.SDD", "OXMAGIC3.SDD",
            "OXMAGIC4.SDD", "OXMATSCH.SDD", "OXMEMCL.SDD", "OXMEMOK.SDD", "OXMEMOP.SDD",
            "OXMONEY.SDD", "OXMOTOR.SDD", "OXMOVE.SDD", "OXPULLER.SDD", "OXSWOFF.SDD", "OXSWON.SDD",
            "OXTHIEF.SDD", "OXTRANS.SDD", "OXTURN.SDD", "OXUNTITL.SDD", "OXWOUOU.SDD", nullptr};

    DatFile* datfile = gi.getDatfile();
    for (int i = 0; oxydsounds[i]; ++i) {
        std::string chunkname = oxydsounds[i];

        if (const OxydLib::ByteVec* snddata = datfile->getChunk(chunkname)) {
            //Log << "Loaded sound file " << chunkname<< "\n";
