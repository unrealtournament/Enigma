...
using MethodMap = std::map<std::string, int (*)(lua_State*)>;
...
void RegisterLuaType(lua_State* L, const std::string& registryKey, CFunction* ops, CFunction* methods,
        MethodMap& methodMap);
 ...
 */
Error CallFunc(lua_State* L, const std::string& funcpath, const enigma::Value& arg, Object* obj,
        bool expectFunction = true);
...
/** Find a Lua script in with the given 'filename' using enigma::FindFile and run it. */
...
/**
 * Try to run the code in 'filename'.
 * If something goes wrong, provide a generic error message and exit Enigma.
 */
...
/**
 * Find all files named 'filename' below 'baseFolder' and run them.
 */
Error DoSubfolderFile(lua_State* L, const std::string& baseFolder, const std::string& filename);
